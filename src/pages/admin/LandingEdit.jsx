...
async function loadLanding() {
  try {
    ...
    if (!landing) {
      errorToast("Landing Page not found.");
    ...
    setThemeColorTo(
      landing.themeColorTo || "#f59e0b"
    );

    setGradientDirection(
      landing.themeGradientDirection || "to right"
    );
    ...
    setCollectName(
      landing.orderForm?.collectName ?? true
    );

    setCollectPhone(
      landing.orderForm?.collectPhone ?? true
    );

    setCollectAddress(
      landing.orderForm?.collectAddress ?? true
    );

    setCollectCity(
      landing.orderForm?.collectCity ?? false
    );

    setCollectNotes(
      landing.orderForm?.collectNotes ?? false
    );
...
Landing Page Theme Color
</h3>

<p className="text-xs text-gray-500 mt-1 mb-3">
Landing Page-এ দাম, হিরো টাইটেল ও accent element এর কালার হিসেবে এটা ব্যবহার হবে। ইচ্ছেমতো যেকোনো কালার দিন, অথবা দুইটা কালার মিক্স (গ্রেডিয়েন্ট) করুন।
</p>
